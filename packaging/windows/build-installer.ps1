...
param(
    ...
    [string]$BuildRoot = (Join-Path ([System.IO.Path]::GetTempPath()) "skywriter-task106-build"),
...
function Assert-SafeBuildPath([string]$Path) {
    ...
    if ($Path -eq $root -or $Path -eq $repositoryRoot -or $leaf -notmatch "skywriter|sw106") {
        throw "BuildRoot must be a dedicated path whose name contains 'skywriter' or 'sw106': $Path"
    }
}
